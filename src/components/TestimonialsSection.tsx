...
const TestimonialsSection: React.FC = () => {
  return <div className="md:py-48 py-[69px]">
      {/* Stars above the heading */}
      <div className="flex justify-center items-center gap-4 mb-8">
        {[...Array(5)].map((_, index) => <Star key={`top-star-${index}`} className="h-6 w-6 fill-gold text-gold" />)}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
        {testimonials.map((testimonial, index) => <TestimonialCard key={index} data={testimonial} />)}
      </div>
    </div>;
};
